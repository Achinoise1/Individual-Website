...
我们可以在 `operations` 列表中添加一个 `RunSQL` 操作来执行自定义的 SQL 语句：
...
`migrations.RunSQL` 接受两个参数，第一个参数是要执行的 SQL 语句，第二个参数是撤销这个迁移时要执行的 SQL 语句。

执行 `migrate` 后，我们可以在数据库中看到 `store_collection` 表中新增了一行数据：

![alt text](03-setup-database/migrate-mysql.png)

此时再执行 `python manage.py migrate store <prev migrate id>` 来撤销这个迁移，再次查看 `store_collection` 表，之前新增的那行数据已经被删除了：

![alt text](03-setup-database/reverse-migrate-mysql.png)
...
![alt text](03-setup-database/mockaroo-set-config.png)

修改完成后点击【PREVIEW】进行预览：

![alt text](03-setup-database/mockaroo-preview-csv.png)

如果预览结果符合预期，我们就可以点击【GENERATE DATA】来下载 CSV 文件：

![alt text](03-setup-database/mockaroo-download-csv.png)

现在我们把导出格式切换为 SQL，并修改表名：

![alt text](03-setup-database/mockaroo-export-sql.png)

再次预览：

![alt text](03-setup-database/mockaroo-preview-sql.png)

如果预览结果符合预期，我们就可以点击【GENERATE DATA】来下载 SQL 文件：

![alt text](03-setup-database/mockaroo-download-sql.png)

将文件拖入到 VS Code 中，选择数据库后选中全部内容，点击右上角的绿色运行按钮执行：

![alt text](03-setup-database/mockaroo-run-sql.png)
...
- [Using MySQL in Django](https://www.bilibili.com/video/BV1eX4y1f7Pz?p=16)
- [Running Custom SQL](https://www.bilibili.com/video/BV1eX4y1f7Pz?p=17)
- [Generating Dummy Data](https://www.bilibili.com/video/BV1eX4y1f7Pz?p=18)